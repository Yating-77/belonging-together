...
-- Option 1: SleepSc ene.vue Option 0: Difficulty falling asleep
...
-- Option 2: SleepScene.vue Option 1: Wakes up frequently and has trouble falling back asleep
INSERT INTO advice (option_id, advice_text, display_order) VALUES 
...
-- Option 5: DietSc ene.vue Option 0: Refuses to try new foods
INSERT INTO advice (option_id, advice_text, display_order) VALUES 
...
-- Option 6: DietSc ene.vueOption 1: Extremely sensitive to food texture or taste
INSERT INTO advice (option_id, advice_text, display_order) VALUES 
...
-- Option 7: DietSc ene.vue Option 2: Needs toys or devices for distraction during meals
INSERT INTO advice (option_id, advice_text, display_order) VALUES 
...
(7, 'Provide positive reinforcement for incremental improvements in focusing on mealtime without devices.', 5);
  ...
  -- Option 8: DietSc ene.vue Option 3: Irregular eating times or difficulty sitting calmly at the table
INSERT INTO advice (option_id, advice_text, display_order) VALUES 
...
-- Option 9: SocialScene.vue Option 0: Plays completely alone
INSERT INTO advice (option_id, advice_text, display_order) VALUES 
...
-- Option 10: SocialScene.vue Option 1: Willing to approach other children but doesn't know how to interact
INSERT INTO advice (option_id, advice_text, display_order) VALUES 
...
-- Option 11: SocialScene.vue Option 2: Often has conflicts with peers or difficulty cooperating
INSERT INTO advice (option_id, advice_text, display_order) VALUES 
...
-- Option 12:  SocialScene.vue Option 3: Focuses only on preferred activities or interests
INSERT INTO advice (option_id, advice_text, display_order) VALUES 
...
WHERE s.name = 'medical_visit';
SELECT ci.*
FROM checklist_items ci
JOIN image_labels il ON ci.id = il.checklist_item_id
WHERE il.image_label IN ('plush toy', 'foot hammock', 'sun hat');
SELECT ci.id, ci.title, s.display_name AS scene
FROM checklist_items ci
JOIN scenes s ON ci.scene_id = s.id
ORDER BY ci.id;
SELECT image_label
FROM image_labels
WHERE checklist_item_id = 7;
...
CREATE TABLE scene_sensory_expectations (
    id SERIAL PRIMARY KEY,
    scene_id INTEGER NOT NULL REFERENCES scenes(id) ON DELETE CASCADE,
    category TEXT NOT NULL,        -- e.g. 'hearing', 'sight', 'smell', 'touch'
...
SET title = 'Packet of Tissues or Wipes',
    ...
    child_reaction = 'Without it, your child may feel messy, overwhelmed, or physically uncomfortable.'
WHERE id = 1;
INSERT INTO image_keywords (checklist_item_id, keyword) VALUES
(1, 'tissue pack'),
(1, 'wet wipes'),
...
-- Make sure you are connected to your 'postgres' database first!
...
-- Add updates for any other venues if needed

SELECT id, venue_title, image_url FROM sensory_venues;
