  ...
  methods: {
    ...
    async fetchCategories() {
      ...
      try {
        ...
        for (const category of categories) {
          ...
          try {
            // Get questions
            const questionsRes = await fetch(`http://localhost:3000/api/questions/${category.id}`);
            ...
            const optionsRes = await fetch(`http://localhost:3000/api/options/${category.id}`);
    ...
    async fetchRecommendations(optionId) {
      try {
        ...
        const response = await fetch(`http://localhost:3000/api/recommendations/${optionId}`);
